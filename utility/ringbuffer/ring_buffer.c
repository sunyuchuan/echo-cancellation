...
static size_t GetBufferReadRegions(RingBuffer* buf, size_t element_count,
                                   void** data_ptr_1, size_t* data_ptr_bytes_1,
                                   void** data_ptr_2,
                                   size_t* data_ptr_bytes_2) {
    const size_t readable_elements = RingBuffer_available_read(buf);
...
int RingBuffer_InitBuffer(RingBuffer* self) {
    if (!self) {
        return -1;
    }
...
size_t RingBuffer_ReadBuffer(RingBuffer* self, void** data_ptr, void* data,
...

    {
        ...
        const size_t read_count = GetBufferReadRegions(
            self, element_count, &buf_ptr_1, &buf_ptr_bytes_1, &buf_ptr_2,
            &buf_ptr_bytes_2);

        if (buf_ptr_bytes_2 > 0) {
            // We have a wrap around when reading the buffer. Copy the buffer
            // data to |data| and point to it.
        ...
        RingBuffer_MoveReadPtr(self, (int)read_count);

        return read_count;
    }
}

size_t RingBuffer_WriteBuffer(RingBuffer* self, const void* data,
                              size_t element_count) {
    if (!self) {
        return 0;
    }
    ...
    {
        const size_t free_elements = RingBuffer_available_write(self);
        ...
        memcpy(
            self->data + self->write_pos * self->element_size,
...
int RingBuffer_MoveReadPtr(RingBuffer* self, int element_count) {
    if (!self) {
        return 0;
    }

    {
        ...
        // instead of "size_t".
        const int free_elements = (int)RingBuffer_available_write(self);
        const int readable_elements = (int)RingBuffer_available_read(self);
...
size_t RingBuffer_available_read(const RingBuffer* self) {
    if (!self) {
        return 0;
    }
...
size_t RingBuffer_available_write(const RingBuffer* self) {
    if (!self) {
        return 0;
    }

    return self->element_count - RingBuffer_available_read(self);
}
