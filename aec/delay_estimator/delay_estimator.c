...
static const int kShiftsAtZero = 10;  // Right shifts at zero binary spectrum.
static const int kShiftsLinearSlope = 3;

static const int kProbabilityOffset = 768;       // 2 in Q9.
static const int kProbabilityLowerLimit = 4096;  // 17 in Q9.
static const int kProbabilityMinSpread = 2048;   // 5.5 in Q9.

FILE* delay_record;
#define BINARY_COMPARE_OFFSET 0

int OpenDelayRecordFile() {
    delay_record = fopen("delay_record", "wb+");
    if (delay_record == NULL) {
        return -1;
    }
    return 0;
}
...
static __inline void DelayEstimator_MeanEstimatorFix(unsigned int new_value,
                                                     int factor,
...
//
static void BitCountComparison(unsigned int binary_vector,
                               const unsigned int* binary_matrix,
                               int matrix_size, int* bit_counts, int offset) {
    int n = offset;

    // Compare |binary_vector| with all rows of the |binary_matrix|
    for (; n < matrix_size; n++) {
        bit_counts[n] = (int)BitCount(binary_vector ^ binary_matrix[n]);
    }
}
...
BinaryDelayEstimatorFarend* DelayEstimator_CreateBinaryDelayEstimatorFarend(
    ...
    if (self != NULL) {
        ...
        self->binary_far_history = malloc(history_size * sizeof(unsigned int));
        ...
        if (malloc_fail) {
            DelayEstimator_FreeBinaryDelayEstimatorFarend(self);
            self = NULL;
        }
    }
...
void DelayEstimator_InitBinaryDelayEstimatorFarend(
    ...
    memset(self->binary_far_history, 0,
           sizeof(unsigned int) * self->history_size);
    memset(self->far_bit_counts, 0, sizeof(int) * self->history_size);
}

void DelayEstimator_AddBinaryFarSpectrum(BinaryDelayEstimatorFarend* handle,
                                         unsigned int binary_far_spectrum) {
    assert(handle != NULL);
    // Shift binary spectrum history and insert current |binary_far_spectrum|.
    memmove(&(handle->binary_far_history[1]), &(handle->binary_far_history[0]),
            (handle->history_size - 1) * sizeof(unsigned int));
...
BinaryDelayEstimator* DelayEstimator_CreateBinaryDelayEstimator(
    ...
    if (self != NULL) {
        ...
        self->mean_bit_counts = malloc(farend->history_size * sizeof(int));
        malloc_fail |= (self->mean_bit_counts == NULL);

        self->bit_counts = malloc(farend->history_size * sizeof(int));
        malloc_fail |= (self->bit_counts == NULL);

        // Allocate memory for history buffers.
        self->binary_near_history =
            malloc((lookahead + 1) * sizeof(unsigned int));
        malloc_fail |= (self->binary_near_history == NULL);

        if (malloc_fail) {
            DelayEstimator_FreeBinaryDelayEstimator(self);
            self = NULL;
        }
    }
...
void DelayEstimator_InitBinaryDelayEstimator(BinaryDelayEstimator* self) {
    ...
    memset(self->binary_near_history, 0, sizeof(int) * self->near_history_size);
...
int DelayEstimator_ProcessBinarySpectrum(BinaryDelayEstimator* self,
    ...
    int value_best_candidate = 32 << 9;  // 32 in Q9, (max |mean_bit_counts|).
    ...
    if (self->near_history_size > 1) {
        ...
        memmove(&(self->binary_near_history[1]),
                &(self->binary_near_history[0]),
                (self->near_history_size - 1) * sizeof(unsigned int));
    ...
    BitCountComparison(binary_near_spectrum, self->farend->binary_far_history,
                       ...
                       BINARY_COMPARE_OFFSET);

    // Update |mean_bit_counts|, which is the smoothed version of |bit_counts|.
    for (i = BINARY_COMPARE_OFFSET; i < self->farend->history_size; i++) {
        // |bit_counts| is constrained to [0, 32], meaning we can smooth with a
        // factor up to 2^26. We use Q9.
        int bit_count = (self->bit_counts[i] << 9);  // Q9.

        // Update |mean_bit_counts| only when far-end signal has something to
        // contribute. If |far_bit_counts| is zero the far-end signal is weak
        // and we likely have a poor echo condition, hence don't update.
        if (self->farend->far_bit_counts[i] > 0) {
            // Make number of right shifts piecewise linear w.r.t.
            // |far_bit_counts|.
            int shifts = kShiftsAtZero;
            shifts -=
                (kShiftsLinearSlope * self->farend->far_bit_counts[i]) >> 4;
            DelayEstimator_MeanEstimatorFix(bit_count, shifts,
                                            &(self->mean_bit_counts[i]));
        }
    }

    // Find |candidate_delay|, |value_best_candidate| and
    // |value_worst_candidate| of |mean_bit_counts|.
    for (i = BINARY_COMPARE_OFFSET; i < self->farend->history_size; i++) {
    ...
    // 2) If the situation is reliable we update |last_delay| if the value of
    // the
    ...
    if ((self->minimum_probability > kProbabilityLowerLimit) &&
        (value_worst_candidate - value_best_candidate >
        ...
        // difference between |value_worst_candidate| and |value_best_candidate|
        // has to be large enough.
        int threshold = value_best_candidate + kProbabilityOffset;
...
int DelayEstimator_binary_last_delay_quality(BinaryDelayEstimator* self) {
    int delay_quality = 0;
    assert(self != NULL);
    // |last_delay_probability| is the opposite of quality and states how deep
    // the minimum of the cost function is. The value states how many
    // non-matching bits we have between the binary spectra for the
    // corresponding delay estimate. The range is thus from 0 to 32, since we
    // use 32 bits in the binary spectra.
